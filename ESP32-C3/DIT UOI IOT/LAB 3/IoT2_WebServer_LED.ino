...
const char* ssid = "mywifissid"; //Enter your WiFi's ID
const char* password = "mypassword"; //Enter your WiFi's PASSWORD
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /4/on") >= 0) {
              Serial.println("Yellow LED On, Blue LED Off"); //print what state the LED is in into the Serial Port
              outputLEDSTATE = "on"; //the yellow LED is ON, the Blue is OFF state
              digitalWrite(outputLED, HIGH);
                 for (int i = 0; i < LEDS_COUNT; i++) {
                      strip.setLedColorData(i, strip.Wheel((i * 256 / LEDS_COUNT+25) & 255));
                    }
                    strip.show();
                    delay(1000);
    
            } else if (header.indexOf("GET /4/off") >= 0) {
              Serial.println("Blue LED On, Yellow LED Off"); //print what state the LED is in into the Serial Port
              outputLEDSTATE = "off"; //the blue LED is ON, the yellow is OFF state
              digitalWrite(outputLED, LOW);
              delay(1000);
              for (int i = 0; i < LEDS_COUNT; i++) {
                    strip.setLedColorData(i, strip.Wheel((i * 256 / LEDS_COUNT+120) & 255));
                  }
                  strip.show();
                  delay(1000);
            }
            ...
            client.println(".button2 {background-color: #fcbb30;}</style></head>");
            ...
            if (outputLEDSTATE=="off") {
              client.println("<p>BLUE = ON, YELLOW = OFF </p>");
              client.println("<p><a href=\"/4/on\"><button class=\"button\">BLUE</button></a></p>");
            } else {
              client.println("<p>YELLOW = ON, BLUE = OFF </p>");
              client.println("<p><a href=\"/4/off\"><button class=\"button button2\">YELLOW</button></a></p>");
            } 
